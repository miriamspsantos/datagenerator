...
Researchers are welcome to contribute their own extensions and data. This documentation is based on [RAPORT-RB-16-14](https://github.com/sysmon37/datagenerator/blob/master/docs/raport-rb-16-14.pdf), which has also been translated to [REPORT-RB-16-14-EN]().
...
* Definition of classes consisting of one or more regions (meta-balls and meta-cubes) with the ability to create integumental regions that fill the space between regions (such fillings are usually related to the majority classes);
...
* Generation of minority examples of all types, as thoroughly described in [5]: *safe*, *borderline*, *rare*, and *outlier*:
    ...
    * **Rare:** Examples forming small disjuncts (consisting of 2-3 examples) lying far from the regions that make up the class: they represent a rare but correct observation;
...
|names.classes|List of names of particular decision classes (comma-separated character strings).<br>`names.classes = MIN, MAJ`|
|names.attributes|List of names of the attributes (comma-separated character strings).<br>`names.attributes = A1, A2`|
|names.decision|Name of the decision atribute.<br>`names.decision = CLASS`|
|classRatio|Proportion of each class. They don't have to sum up tp 1 or 100, the generator determines the proportion and converts it to the number of points in each class.<br>`classRatio = 1:9`|
...
|defaultRegion.noOutlierZone|Absolute width of a zone in which *rare* and *outlier* points cannot appear. This zone is located around the zone with *borderline* points. More information on [REPORT-RB-16-14-EN]().<br>`defaultRegion.noOutlierZone =1.5`|
...
|learnTestPairs|Number of training/test pairs. If not specified, only the trainig set is generated. If the `learnTestRatio` parameter is given and the `learnTestPairs` is ommited, then the generator will report an error and stop running.<br>`learnTestPairs = 5`|
|fileName.learn|Template for the name of the training sets. It should contain the suffix `-%d` which will be replaced by consecutive indeces from 1 to `learnTestPairs`. It is mandatory if the `learnTestRatio` is given, otherwise it can be omitted.<br>`fileName.learn = paw3-2d-learn-%d.arff`|
|fileName.test|Template for the name of the test sets. It should contain the suffix `-%d` which will be replaced by consecutive indeces from 1 to `learnTestPairs`. It is mandatory if the `learnTestRatio` is given, otherwise it can be omitted.<br>`fileName.test = paw3-2d-learn-%d.arff`|
...
Additional information regarding the data generation (design and implementation details, main packages and classes, scheme of generator operations) is based on [RAPORT-RB-16-14](https://github.com/sysmon37/datagenerator/blob/master/docs/raport-rb-16-14.pdf), and translated to [REPORT-RB-16-14-EN](). A list of parameters, their description, possible values, requirements, and example of use is also provided, for ease of use ([Generator-Parameters.xls]()).
